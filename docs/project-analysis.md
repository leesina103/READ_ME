...
로그인과 회원가입 화면에는 `<form>`, `<label>`, `autoComplete`가 적용되어 있다. 제출 핸들러와 상태 관리는 아직 없어 버튼을 눌러도 실제 인증 동작은 수행되지 않는다.
...
- [x] Noto Sans KR·Noto Serif KR 웹폰트 적용
- [x] 로그인 및 회원가입을 실제 `<form>` 구조로 변경
- [ ] 폼 유효성 검사 및 오류 메시지 추가
- [x] 메인과 모임 페이지의 기수·회차 데이터 통합
- [x] `metadataBase` 및 Open Graph 메타데이터 추가
- [x] 카카오톡 링크 미리보기용 대표 이미지 연결
- [ ] 테스트 환경 및 핵심 기능 테스트 추가

## 7. 구현 전에 확정할 사항

현재 화면은 **한 기수에 네 회차가 포함되고, 사용자는 기수 전체에 신청한다**는 구조로 통일되어 있다. 메인과 모임 페이지는 같은 공통 데이터 모듈을 사용한다.
