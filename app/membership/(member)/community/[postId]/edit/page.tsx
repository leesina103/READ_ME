...
export default async function EditCommunityPostPage({ params }: EditCommunityPostPageProps) {
  ...
  if (!user) notFound();
  const { data: post, error } = await supabase.from("community_posts").select("id, user_id, category, title, content, book_title, book_author, external_url").eq("id", postId).eq("user_id", user.id).maybeSingle();
