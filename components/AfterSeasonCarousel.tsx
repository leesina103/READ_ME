...
const afterSeasonItems = [
  {
    ...
    image: "/after-talk-preview-v5.jpg",
    imageHeight: 571,
    alt: "질문과 답변을 다시 볼 수 있는 READ ME 온라인 대화 화면"
  },
  {
  ...
  },
  {
    ...
    image: "/after-membership-preview-v5.jpg",
    imageHeight: 598,
    alt: "온라인 대화와 멤버 활동을 모아둔 READ ME 멤버십 홈 화면"
  },
  {
    timing: "전 기수와 함께 · 온라인",
    title: "멤버십 커뮤니티",
    ...
    image: "/after-community-preview-v5.jpg",
    imageHeight: 648,
    ...
    <div className="after-season">
      ...
      <ul ref={trackRef} className="after-season__track" onScroll={handleScroll} tabIndex={0} aria-label="한 기수 이후 이어지는 활동">
        {afterSeasonItems.map((item, index) => {
          const isScreenCapture = item.image.startsWith("/after-");

          return (
            <li key={item.title}>
              <figure className={isScreenCapture ? "after-season__figure--screen" : undefined}>
                <Image
                  src={item.image}
                  alt={item.alt}
                  width={isScreenCapture ? 606 : 960}
                  height={item.imageHeight}
                  sizes="(max-width: 820px) 82vw, 560px"
                />
              </figure>
