...
import { readingGroupStories } from "@/data/stories";
import { useHorizontalCarousel } from "@/components/useHorizontalCarousel";

export function StoryCarousel() {
  const { activeIndex, canMoveNext, canMovePrevious, handleScroll, moveTo, trackRef } = useHorizontalCarousel(readingGroupStories.length);

  return (
    <div className="story-carousel">
      <div className="story-carousel__controls">
        <p aria-live="polite"><strong>{String(activeIndex + 1).padStart(2, "0")}</strong> / {String(readingGroupStories.length).padStart(2, "0")}</p>
        <div>
