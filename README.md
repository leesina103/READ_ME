...
1. `cohorts` 테이블에 `name`(기수명), `starts_at`(기수 시작 시각)을 등록합니다. `starts_at`은 온라인 대화방 1주차 공개 시각으로도 쓰이므로, 매주 열고 싶은 요일과 시각(예: 월요일 0시)으로 맞춰 넣습니다.
...
멤버 소개(`/membership/members`)는 회원이 참여한 기수의 동료 소개를 모아 보여줍니다. `/onboarding`에서 받은 자기소개와 동료들에게 하고 싶은 말을 기수별로 묶어 내보내며, 현재 기수를 맨 위에 두고 참여했던 지난 기수를 이어 붙입니다. 프로필 표는 여전히 본인 행만 조회할 수 있고, 공유가 필요한 항목만 `list_member_directory()` 함수로 내보냅니다. 이 함수는 활성 멤버십과 참여 기수를 확인하므로 다른 기수의 소개는 반환하지 않습니다. 회원은 `/my`에서 두 글을 언제든 고칠 수 있습니다.

온라인 대화방(`/membership/talk`)은 `cohorts.starts_at`을 1주차 공개 시각으로 삼아 매주 같은 요일·시각에 다음 주차를 엽니다. 그룹마다 오프라인 모임 요일이 달라도 대화방은 기수 단위로 같은 시각에 열립니다. 아직 열리지 않은 주차는 목록에 잠금과 공개 날짜로 표시되고, 주소로 직접 들어가면 목록으로 돌려보내며, 데이터베이스의 답변 저장 함수도 거절합니다. 지난 기수 대화방은 읽기 전용이라 모든 주차를 열어둡니다. 시작 시각을 바꾸면 이후 주차 공개일도 함께 바뀝니다.
